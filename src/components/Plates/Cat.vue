<template>
  <!-- <div id="cat_box"> -->
    <section style="width: 100vw;height: 100vh;position: fixed;left: 0;top: 0;z-index: -1;" id="world" />
  <!-- </div> -->
</template>

<script lang="ts" setup>
import { onMounted } from "vue";
import { Cat } from '@/libs/threeCat/Cat'

window.innerHeight= window.outerHeight * 1.55
...
function initScreenAnd3D() {
  ...
  HEIGHT = window.innerHeight ;
    ...
    const box: any = document.getElementById('cat_box');
...
WoolVert.prototype.update = function() {
    ...
    const nx = this.x + ((this.x - this.oldx) * .9) + this.fx;
    const ny = this.y + ((this.y - this.oldy) * .9) + this.fy;
    this.oldx = this.x;
    this.oldy = this.y;
    this.x = nx;
    this.y = ny;
...
const Constraint = function(p1, p2) {
    this.p1 = p1;
    this.p2 = p2;
...
function getBallPos() {
    ...
    vector.set(
        (mousePos.x / WIDTH) * 2 - 1, -(mousePos.y / HEIGHT) * 2 + 1,
        0.1);
